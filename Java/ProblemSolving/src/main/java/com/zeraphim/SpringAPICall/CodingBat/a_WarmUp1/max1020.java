package com.zeraphim.SpringAPICall.CodingBat.a_WarmUp1;

/*

Given 2 positive int values, return the larger value that is in the range 10..20 inclusive, or return 0 if neither is in that range.

max1020(11, 19) → 19
max1020(19, 11) → 19
max1020(11, 9) → 11

 */

public class max1020 {
    public static void main(String[] args) {
        System.out.println(max1020(11, 19));
        System.out.println(max1020(19, 11));
        System.out.println(max1020(11, 9));
        System.out.println(max1020(9, 21));
        System.out.println(max1020(10, 21));
        System.out.println(max1020(9, 11));
        System.out.println(max1020(23, 10));
        System.out.println(max1020(20, 10));
        System.out.println(max1020(7, 20));
        System.out.println(max1020(17, 16));
    }

    public static int max1020(int a, int b) {
        boolean numOneInRange10To20 = a >= 10 && a <= 20;
        boolean numTwoInRange10To20 = b >= 10 && b <= 20;

        boolean numOneCheckValidity = !numTwoInRange10To20 && numOneInRange10To20;
        boolean numTwoCheckValidity = !numOneInRange10To20 && numTwoInRange10To20;

        if ((a > b && numOneInRange10To20) || numOneCheckValidity) {
            return a;
        } else if ((b > a && numTwoInRange10To20) || numTwoCheckValidity) {
            return b;
        }

        return 0;
    }
}